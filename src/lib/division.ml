...
let attempt_div ~order ~fs ~qs ~p =
  ...
  in
  let+ quotient, i = List.find_mapi find_div fs in
  let q_i = List.nth qs i in
  let f_i = List.nth fs i in
  Polynomial.(Util.List.set_at_idx i (q_i + quotient) qs, p - (quotient * f_i))
...
let top ~order f fs =
  ...
  in
  let s = List.length fs in
  let qs = List.init s (fun _ -> Polynomial.zero) in
