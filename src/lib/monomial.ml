...
let equal (m1 : t) (m2 : t) = List.equal ( = ) m1 m2

open Stdlib

(* By convention, we sort individual vars in lex ordering,
   i.e. x < y < z. We maintain the invariant that all monomials are sorted. *)
let sort_mon (m : t) = List.sort (fun (v1, _exp) (v2, _exp) -> compare v1 v2) m

let of_string str =
  let rec parse_chars = function
    | [] -> []
    | chars ->
        let var, rest = (List.hd chars, List.tl chars) in
        let exp_str, rest =
          Util.List.take_drop_while Util.String.is_digit rest
        in
        let exp =
          if exp_str = [] then 1
          else int_of_string (Util.String.of_list exp_str)
        in
  ...
  str |> Util.String.to_list |> parse_chars |> sort_mon
...
let rec collapse = function
  ...
  | (x, e) :: rest ->
      ...
      let matches, others = List.partition matches_id rest in
      let sum_exp = List.fold_left (fun sum (_x, e) -> sum + e) e matches in
...
let ( / ) m1 m2 =
  ...
  let negate m = List.map (fun (v, e) -> (v, -e)) m in
  let result = collapse @@ m1 @ negate m2 in
  if List.exists (fun (_v, e) -> e < 0) result then None else Some result
...
let ord m = List.fold_left (fun o (_v, e) -> o + e) 0 m
let const = []
...
module Order = struct
  ...
  let rec lex (m1 : t) (m2 : t) =
    ...
    | (v1, e1) :: rest1, (v2, e2) :: rest2
      when Char.equal v1 v2 && Int.equal e1 e2 ->
        lex rest1 rest2
    | (v1, e1) :: _, (v2, e2) :: _ when Char.equal v1 v2 -> compare e1 e2
  ...
  let grevlex m1 m2 = Int.neg @@ grlex (List.rev m1) (List.rev m2)
